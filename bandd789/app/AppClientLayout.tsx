...
export default function AppClientLayout({ children }: { children: React.ReactNode }) {
  ...
  const { currentView, selectedPageSlug } = React.useMemo(() => {
    const segments = pathname.split('/').filter(Boolean);

    if (pathname === '/') {
        return { currentView: 'list' as View, selectedPageSlug: null };
    }
    ...
    if (segments[0] === 'admin') {
        return { currentView: 'admin' as View, selectedPageSlug: null };
    }

    // Default to custom page for other top-level routes
    return { currentView: 'customPage' as View, selectedPageSlug: segments[0] || null };
  }, [pathname]);
