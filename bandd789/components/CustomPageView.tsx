    ...
    const [formData, setFormData] = useState({
      ...
      return (
        <form onSubmit={handleSubmit}>
            ...
            <button 
                type="submit" 
                className="w-full md:w-auto bg-primary text-white font-bold py-3 px-12 rounded-lg hover:bg-secondary transition-colors duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1"
            >
                ส่งข้อมูลนัดหมาย
            </button>
            </div>
...
const CustomPageView: React.FC<CustomPageViewProps> = ({ page, onAddLead }) => {
  const fullWidthSlugs = ['construction-process', 'system-spec', 'extra-services'];
  const isFullWidth = fullWidthSlugs.includes(page.slug);
  const shouldShowCta = fullWidthSlugs.includes(page.slug);
