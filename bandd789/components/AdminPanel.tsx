...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  loginLogs, onLogout,
  calendarEvents = [], onAddCalendarEvent = (_: Omit<CalendarEvent, 'id'>) => {}, onUpdateCalendarEvent = (_: CalendarEvent) => {}, onDeleteCalendarEvent = (_: string) => {},
  galleryAlbums = [], onAddGalleryAlbum = (_: Omit<GalleryAlbum, 'id'>) => {}, onUpdateGalleryAlbum = (_: GalleryAlbum) => {}, onDeleteGalleryAlbum = (_: string) => {},
}) => {
  ...
  const renderTestimonialTable = () => (
    // ... (No change to Testimonial Table)
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
  ...
  const renderNotificationTable = () => (
    // ... (No change to Notification Table)
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <tr key={event.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4">
                  <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full 
                    ${event.type === 'construction' ? 'bg-orange-100 text-orange-800' : 
                      event.type === 'handover' ? 'bg-green-100 text-green-800' : 
                      event.type === 'holiday' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>
                      {event.type}
                  </span>
              </td>
              <td className="px-6 py-4 text-sm font-medium">
                <div className="flex space-x-3">
                  <button onClick={() => setFormState({ type: 'calendar', data: event })} className="text-indigo-600 hover:text-indigo-900">Edit</button>
                  <button onClick={() => onDeleteCalendarEvent(event.id)} className="text-red-600 hover:text-red-900">Delete</button>
                </div>
              </td>
            </tr>
          ))}
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Album Title</th>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {galleryAlbums.map(album => (
            <tr key={album.id} className="hover:bg-gray-50">
              <td className="px-6 py-4"><img src={album.coverImage} alt={album.title} className="h-12 w-16 object-cover rounded"/></td>
              <td className="px-6 py-4">
                  <div className="text-sm font-medium text-gray-900">{album.title}</div>
                  <div className="text-xs text-gray-500">{album.description}</div>
              </td>
              <td className="px-6 py-4 text-sm text-gray-500">{album.images.length} items</td>
              <td className="px-6 py-4 text-sm text-gray-500">{album.date}</td>
              <td className="px-6 py-4 text-sm font-medium">
                <div className="flex space-x-3">
                  <button onClick={() => setFormState({ type: 'gallery', data: album })} className="text-indigo-600 hover:text-indigo-900">Edit</button>
                  <button onClick={() => onDeleteGalleryAlbum(album.id)} className="text-red-600 hover:text-red-900">Delete</button>
                </div>
              </td>
            </tr>
          ))}
  ...
  const renderActiveContent = () => {
    switch(activeTab) {
      ...
      case 'gallery': return renderGalleryTable();
      default: return null;
    }
  }
  ...
  const getAddNewButtonText = () => {
    switch (activeTab) {
      ...
      case 'calendar': return 'Add New Event';
      case 'gallery': return 'Add New Album';
    ...
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <aside className={`bg-white shadow-xl fixed inset-y-0 left-0 z-50 w-64 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static flex-shrink-0`}>
          ...
          <div className="p-4 space-y-1 overflow-y-auto h-[calc(100vh-4rem)]">
              ...
              <SidebarItem id="calendar" label="Calendar" icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>} />
              <SidebarItem id="gallery" label="Photo Gallery" icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>} />
      ...
      {formState.type === 'calendar' && (
        <CalendarForm
          event={formState.data}
          onSave={(data) => {
            if (formState.data) {
              onUpdateCalendarEvent({ ...data, id: formState.data.id });
            } else {
              onAddCalendarEvent(data);
            }
            handleCloseForm();
          }}
          onClose={handleCloseForm}
      ...
      {formState.type === 'gallery' && (
        <GalleryForm
          album={formState.data}
          onSave={(data) => {
            if (formState.data) {
              onUpdateGalleryAlbum({ ...data, id: formState.data.id });
            } else {
              onAddGalleryAlbum(data);
            }
            handleCloseForm();
          }}
          onClose={handleCloseForm}
