...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  // ... (Keep other render functions similar but wrapped in the nicer table structure like above)
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {pages.map(page => (
            <tr key={page.id} className="hover:bg-gray-50">
              <td className="px-6 py-4">
                  {page.imageUrl ? <img src={page.imageUrl} alt={page.title} className="h-10 w-16 object-cover rounded" /> : <span className="text-xs text-gray-400">No Image</span>}
              </td>
              <td className="px-6 py-4 text-sm font-medium text-gray-900">{page.title}</td>
              <td className="px-6 py-4 text-sm font-mono text-gray-500 bg-gray-50 rounded w-max px-2 py-0.5">/{page.slug}</td>
              <td className="px-6 py-4 text-sm font-medium">
                <div className="flex space-x-3">
