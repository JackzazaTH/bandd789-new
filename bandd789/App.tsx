...
import { HomeDesign, Banner, Lead, Article, SEOData, View, CustomPage, PromoCard, Testimonial, SiteNotification, PopupModalContent, SiteInfo, Toast, PortfolioProject, LeadStatus } from './lib/types';
  ...
  const [storedValue, setStoredValue] = useState<T>(() => {
    ...
    try {
      ...
      return JSON.parse(item, (k, v) => {
        // Reviver to handle date strings from JSON
...
const CookieConsent: React.FC<{ onAccept: () => void }> = ({ onAccept }) => {
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gray-800 bg-opacity-90 backdrop-blur-sm text-white p-4 z-[100] animate-slide-in-up">
      <div className="container mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
        <p className="text-sm text-center sm:text-left">
          เว็บไซต์นี้ใช้ Local Storage เพื่อบันทึกข้อมูลและปรับปรุงประสบการณ์การใช้งานของคุณ การใช้งานเว็บไซต์ต่อถือว่าคุณยอมรับนโยบายการจัดเก็บข้อมูลของเรา
        </p>
        <button
          onClick={onAccept}
          className="bg-primary text-white font-bold py-2 px-6 rounded-lg hover:bg-secondary transition-colors duration-300 flex-shrink-0"
        >
          ยอมรับ
        </button>
      </div>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const consent = localStorage.getItem('ddhouse_cookie_consent');
    if (consent !== 'true') {
  ...
  const handleAcceptCookies = () => {
    localStorage.setItem('ddhouse_cookie_consent', 'true');
    setShowCookieConsent(false);
    addToast('ขอบคุณที่ยอมรับนโยบายของเรา', 'info');
  };
  ...
  const navigateTo = (view: View, slug?: string) => {
    ...
    if (view === 'detail' && slug) setSelectedHomeId(slug);
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'portfolio':
        return <PortfolioPage projects={portfolioProjects} onNavigate={navigateTo} testimonials={testimonials} onPlayVideo={setVideoModalUrl} />;
